...
def walk_cpu(session, oid):
    ...
    for i in range(n):
        id, uptime, cpu, timestamp = (int(x.value) for x in session.walk("{oid}.1.1.{i}".format(oid=oid, i=i)))
        yield (id, uptime, cpu/Decimal(100), timestamp)

def walk_mem(session, oid):
    n = int(session.get("{oid}.2.0".format(oid=oid)).value)
    for i in range(n):
        id, used, percent, total, timestamp = (int(x.value) for x in session.walk("{oid}.2.1.{i}".format(oid=oid, i=i)))
        yield (id, used, percent / Decimal(100), total, timestamp)
...
def add_cpu(ph, metrics):
    ...
    for id, uptime, percent, timestamp in metrics:
        ...
        ph.add_metric('cputs_{0}'.format(id), timestamp)
    if cpucount:
...
def add_mem(ph, metrics):
    ...
    for id, used, percent, total, timestamp in metrics:
        ...
        ph.add_metric('memts_{0}'.format(id), timestamp)
    if summem:
